...
export async function getBookingsForUser(userId: string) {
    ...
    const { data: accommodationDetails, error: accommodationError } = hotelsBookingIds.length > 0 ? await supabase.from('accommodation_booking.details').select(`
            booking_id,
            room_type_id,
            check_in,
            check_out,
    ...
    return { bookings, RestaurantDetails, accommodationDetails };
}